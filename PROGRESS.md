...
## Current step
M1 — Types & schema. Defining ObservationRecord, Baseline, DeviationResult, Tier + validateObservation().

## Next action (resume here)
Write src/types/*.ts (ObservationRecord, Baseline, DeviationResult, Tier) and src/types/validateObservation.ts, plus tests/unit fixtures (do not execute tests this session per instruction — write only).
